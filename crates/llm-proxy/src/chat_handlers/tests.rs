...
impl MockChatUpstream {
    fn start() -> Self {
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                if request.windows(4).any(|w| w == b"\r\n\r\n") {
                    ...
                    tx.send(serde_json::from_slice(body).expect("request json")).expect("send body");
                    break;
                }
            }
        ...
        Self { base_url: format!("http://{address}/v1"), body: rx }
    }
}
...
impl MockChatSseUpstream {
    fn start() -> Self {
        ...
        let address = listener.local_addr().expect("mock chat SSE upstream address");
        let (tx, rx) = mpsc::channel();
        thread::spawn(move || {
...
async fn chat_completions_rewrites_model_and_preserves_fields() {
    ...
    let records: Vec<Value> = log_contents.lines().filter_map(|l| serde_json::from_str(l).ok()).collect();
...
async fn chat_completions_streaming_forwards_events_and_logs_them() {
    ...
    let stream_events: Vec<&Value> = records.iter().filter(|r| r["record_type"] == "stream_event").collect();
    assert!(stream_events.len() >= 3, "expected at least 3 stream events, got {}", stream_events.len());
