...
async fn streaming_forwards_events_and_logs_them() {
    ...
    assert_eq!(
        stream_events[0]["data_raw"],
        r#"{"type":"response.created","id":"resp-1"}"#
    );
    assert_eq!(stream_events[1]["event"], "");
    assert_eq!(
        stream_events[1]["data_raw"],
        r#"{"type":"response.output_text.delta","delta":"Hello"}"#
    );
    assert_eq!(stream_events[2]["event"], "");
    assert_eq!(
        stream_events[2]["data_raw"],
        r#"{"type":"response.completed","usage":{"input_tokens":10,"output_tokens":5,"total_tokens":15}}"#
    );
