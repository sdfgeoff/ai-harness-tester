...
async fn messages_non_streaming(
    ...
    let response_value = serde_json::from_slice::<Value>(&response_body).unwrap_or_else(
        |_| Value::String(String::from_utf8_lossy(&response_body).to_string()),
    );

    log_end(&state.log, &request_id, start_instant.elapsed().as_millis() as u64,
        "generation", "POST", "/v1/messages", &original_model, &state.model_name,
        status, Some(&response_value), usage.as_ref(), None).await;

    let content_type = response_value.as_object()
...
async fn forward_sse(
                ...
                Ok(chunk) => {
                    ...
                    for line in String::from_utf8_lossy(&chunk).lines() {
                        if line.starts_with("data: ") {
                            let data_raw = &line[6..];
                            if data_raw.is_empty() || data_raw == "[DONE]" { continue; }
                            log_record(&log, &json!({
    ...
    Response::builder()
        ...
        .body(Body::from_stream(crate::streaming::ReceiverStreamCompat::new(rx)))
        .unwrap_or_else(|_| StatusCode::INTERNAL_SERVER_ERROR.into_response())
}
...
async fn log_end(
    ...
    if let Some(body) = response_body {
        if let Some(obj) = record.as_object_mut() { obj.insert("response_body".to_owned(), body.clone()); }
    }
    if let Some(u) = usage {
        if let Some(obj) = record.as_object_mut() { obj.insert("usage".to_owned(), u.clone()); }
    } else {
        if let Some(obj) = record.as_object_mut() { obj.insert("usage".to_owned(), Value::Null); }
    }
    if let Some(e) = error {
        if let Some(obj) = record.as_object_mut() { obj.insert("error".to_owned(), Value::String(e.to_owned())); }
    } else {
        if let Some(obj) = record.as_object_mut() { obj.insert("error".to_owned(), Value::Null); }
    }
